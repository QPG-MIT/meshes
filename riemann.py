...
def get_err(N, mu, sig, eta, err3=0, ct=20, method='reck', share_u=True, share_dp=True):
    r"""
    ...
    :param eta: Coupling angle for third splitter.  Prominent choices: 0 -> MZI, pi/2 -> 3-MZI, pi -> MZI+X
    :param err3: Whether third splitter has random errors or not.
    ...
    """
    if np.iterable(N):
        return np.array([get_err(N_i, mu, sig, eta, err3, ct, method, share_u, share_dp) for N_i in N])
    print (f"N = {N}")
    err_c = np.zeros([ct, len(mu)])
    dj = max(len(mu)//50, 1); di = max(ct//50, 1)
    dp_mask = np.array([[1,1,1]]); dp_mask[0,2] = err3
    def dp_random():
