...
__device__ void dp_mzi(const float *p, const float *s, const complex64 dT[4], float *dp)
{
    ...
    dp0 = (dT[0] * complex64(C1, S1) * (0.5f * (Cm-Sp) * complex64(-S,  C)) + 
           dT[1] * complex64(C , S ) * (0.5f * (Cp+Sm) * complex64(-C, -S)) + 
           dT[2] * complex64(C1, S1) * (0.5f * (Cp-Sm) * complex64(-C, -S)) + 
           dT[3] * complex64(C , S ) * (0.5f * (Cm+Sp) * complex64( S, -C))).real() * 2;
    dp1 = (dT[0] * complex64(C1, S1) * (complex64(-Cm*S, -C*Sp)) + 
           dT[2] * complex64(C1, S1) * (complex64(-C*Cp,  S*Sm))).real() * 2;
    atomicAdd(&dp[0], dp0);
    atomicAdd(&dp[1], dp1);
}
                                     ...
                                     complex64 &temp, bool cond)
{
    ...
    temp = T[0]*du1 + T[1]*du2 + dT[0]*u1 + dT[1]*u2;
...
__device__ __inline__ void matmult_bk(const complex64 T[4], complex64 dT[4], 
                                      complex64 &u1, complex64 &u2, complex64 &dJdu1, complex64 &dJdu2, 
                                      complex64 &temp, bool cond)
{
    // u_i -> (T^dag)_{ij} u_j = (T_{ji})^* u_j
    temp = conj(T[0])*u1 + conj(T[2])*u2;
    u2   = conj(T[1])*u1 + conj(T[3])*u2;
    if (cond)
        u1 = temp;
    // dJ/dT_{ij} = (dJ/du_i)_{out} (u_j)_{in} = (dJ/du_i^*)_{out}^* (u_j)_{in} 
    atomicAdd(&dT[0], conj(dJdu1)*u1); atomicAdd(&dT[1], conj(dJdu1)*u2); 
    atomicAdd(&dT[2], conj(dJdu2)*u1); atomicAdd(&dT[3], conj(dJdu2)*u2); 
    //dT[0] += conj(dJdu1)*u1;  dT[1] += conj(dJdu1)*u2;
    //dT[2] += conj(dJdu2)*u1;  dT[3] += conj(dJdu2)*u2;
    // dJ/du_i^* -> (T^*)_{ij} dJ/du_j = (T_{ji}) dJ/du_j^*
    temp  = conj(T[0])*dJdu1 + conj(T[2])*dJdu2;
    dJdu2 = conj(T[1])*dJdu1 + conj(T[3])*dJdu2;
    if (cond)
        dJdu1 = temp;
}
